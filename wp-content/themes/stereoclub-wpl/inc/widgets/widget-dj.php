...
add_action('widgets_init', create_function('', 'return register_widget("wplook_dj_widget");'));
class wplook_dj_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'wplook_dj_widget',
			'WPlook DJ',
			array( 'description' => __( 'A widget for displaying DJs', 'wplook' ), )
		);
	}
	...
	public function form( $instance ) {
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('nr_posts'); ?>"> <?php _e('Number of DJs:', 'wplook'); ?> </label>
				...
				<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;"> <?php _e('Number of DJ you want to display', 'wplook'); ?></p>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('read_more_link'); ?>"> <?php _e('URL to all DJ page:', 'wplook'); ?> </label>
				...
				<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;"> <?php _e('URL to all djs', 'wplook'); ?></p>
			</p>
	...
	public function widget( $args, $instance ) {
		...
		<?php
			...
			$dj = null;
			$dj = new WP_Query( $args );
		?>

			<?php if( $dj->have_posts() ) : ?>
			
				<!-- Widget DJ -->
				<aside id="resident-dj-<?php the_ID(); ?>" class="widget residentdj">
					...
					<ul>
						<?php while( $dj->have_posts() ) : $dj->the_post(); ?>
						<?php 
							...
							<li>
								<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
									...
									<?php if ( wplook_custom_taxonomies_terms_links() ) { ?>
										<h2 class="entry-description"><?php echo wplook_custom_taxonomies_terms_links(); ?></h2>
									<?php } ?>
